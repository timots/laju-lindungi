...
const Navbar = () => {
  ...
  const getLocation = async () => {
    if ('geolocation' in navigator) {
      try {
        ...
        const currencyCode = currencyData[0]?.currencies
          ? Object.keys(currencyData[0].currencies)[0]
          : 'Unknown';
  ...
  if (isLoading) {
    return (
      <div className="flex h-16 items-center justify-center">Loading...</div>
    );
  }
